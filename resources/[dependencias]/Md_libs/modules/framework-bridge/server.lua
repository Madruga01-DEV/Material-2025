Md.file.load('framework-bridge.overwrite-functions')
...
function User:init()
  ...
  elseif Md.framework:is("VORP") then
   self.data = Md.framework.core.getUser(self.source).getUsedCharacter
  elseif Md.framework:is("RedEM2023") then
    self.data = Md.framework.core.GetPlayer(self.source)
  elseif Md.framework:is("RedEM") then
    local user = promise.new()
    TriggerEvent('redemrp:getPlayerFromId', self.source, function(_user)
      user:resolve(_user)
    end)
    self.data = Citizen.Await(user)
  elseif Md.framework:is("QBR") then
    self.data = Md.framework.core:GetPlayer(self.source)
  elseif Md.framework:is("RSG") or Md.framework:is("QR") then
    self.data = Md.framework.core.Functions.GetPlayer(self.source)
  elseif Md.framework:is('RPX') then
    self.data = Md.framework.core.GetPlayer(self.source)
  end
end
...
function User:getMoney(moneyType)
  ...
  elseif Md.framework:is("QBR") or Md.framework:is("RSG") or Md.framework:is("QR") then
    if moneyType == 0 then
...
function User:getJob()
  ...
  elseif Md.framework:is("VORP") or Md.framework:is('RedEM2023') then
...
function User:getRPName()
  ...
  if Md.framework:is("VORP") or Md.framework:is("RedEM2023") or Md.framework:is("RedEM") then
...
function FrameworkClass:init()
  ...
  eprint('No compatible Framework detected. Please contact Madruga Dev on discord')
end
...
local function standardizeSkinKey(category)
  local framName = Md.framework:get()
...
Md.framework = FrameworkClass:new()
return Md.framework
