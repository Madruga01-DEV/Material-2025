...
CreateThread(function()
  ...
  if not GetResourceMetadata(resourceName, 'ui_page') then
    eprint('WARNING ! NUI page is not defined. To use Md Menu, add ui_page "nui://Md_libs/nui/menu/index.html" inside your fxmanifest.lua')
    eprint('WARNING ! NUI page is not defined. To use Md Menu, add ui_page "nui://Md_libs/nui/menu/index.html" inside your fxmanifest.lua')
    eprint('WARNING ! NUI page is not defined. To use Md Menu, add ui_page "nui://Md_libs/nui/menu/index.html" inside your fxmanifest.lua')
    eprint('WARNING ! NUI page is not defined. To use Md Menu, add ui_page "nui://Md_libs/nui/menu/index.html" inside your fxmanifest.lua')
    eprint('WARNING ! NUI page is not defined. To use Md Menu, add ui_page "nui://Md_libs/nui/menu/index.html" inside your fxmanifest.lua')
...
local function SendNUIMessage(data)
  ...
  if hasMainScript() then
    return Md.menu.exports.sendNUIMessage(data)
  end
...
function Md.menu.addItem(id,p,item) menus[id]:addItem(p,item) end
...
function Md.menu.addItems(id,items) menus[id]:addItems(items) end

function MenuClass:updateItem(index,key,value)
  self.items[index][key] = value
end
function Md.menu.updateItem(id,index,key,value) menus[id]:updateItem(index,key,value) end

function MenuClass:refresh()
  if hasMainScript() then
    return Md.menu.exports.refreshMenu(self.id)
  end
...
function Md.menu.refresh(id) menus[id]:refresh() end
...
function Md.menu.reset(id) menus[id]:reset() end
...
function Md.menu.sort(id,first,last) menus[id]:sort(first,last) end

function MenuClass:send(reset)
  if hasMainScript() then
    Md.menu.exports.sendMenu(clearForCopy(self))
    return
  end
  local datas = table.clearForNui(self)
  if reset == nil then
...
function Md.menu.send(id) menus[id]:send() end

---@param id string Unique ID of the menu
---@param data? MenuClass
function Md.menu.create(id, data)
  if not id then
...
function Md.menu.setCurrentMenu(id, keepHistoric, resetMenu)
...
Md.timeout.loop(1000,LoopDisableKeys)
...
function Md.menu.show(show, keepInput, hideRadar)
  CreateThread(function()
    ...
    if not nuiShow then
      timeoutClose = Md.timeout.set(150, function()
...
function Md.menu.updateLang(lang)
  SendNUIMessage({
    event="updateLang",
    lang = lang
  })
...
function Md.menu.updateVolume(volume)
  SendNUIMessage({
    event="updateVolume",
    volume = volume
  })
...
function Md.menu.get(id)
  return menus[id]
end

function Md.menu.set(id,menu)
  menus[id] = menu
end

function Md.menu.setMainScript(name)
  resourceNUI = name
end
...
function Md.menu.onChange(cb)
  table.insert(Md.menu.listeners,{
...
AddEventHandler('onResourceStop', function(resourceName)
  local i = 1
  while i <= #Md.menu.listeners do
    if Md.menu.listeners[i].resource == resourceName then
      table.remove(Md.menu.listeners,i)
    else
      i += 1
    end
  end
...
local function menuNUIChange(data)
  ...
  for _, listener in ipairs(Md.menu.listeners) do
...
RegisterNUICallback('updatePreview', function(data, cb)
  cb('ok')
  Md.timeout.delay('menuNUIChange', 100, function()
    menuNUIChange(data)
  end)
end)
...
function MenuData.Open(type, namespace, name, data, submit, cancel, change, close)
  ...
  local nuiMenu = Md.menu.create(name, menu)
  ...
  Md.menu.setCurrentMenu(name,true,true)
  Md.menu.show(true, true, false)
  return menu
end
...
Md.menu.bridgeOldMenu = MenuData

exports('Md_menu_get', function()
  return Md.menu
end)
exports('Md_menu_get_current_data', function()
  return currentData
end)
return Md.menu
