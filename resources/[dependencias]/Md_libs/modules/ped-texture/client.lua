...
---@param data table
function Md.pedTexture.getOverlayAssetFromId(isMale,category,data)
...
---@param data table
function Md.pedTexture.apply(ped,layerName,data)
  ...
  local category = Md.pedTexture.categories[layerName]
  if not category then
    return print('No texture category for layer: '..layerName)
  end
  pedsTextures[ped] = pedsTextures[ped] or Entity(ped).state['Md_pedTexture'] or {}
  pedsTextures[ped][category] = pedsTextures[ped][category] or {layers = {}}


  if data.id then
    data.albedo = Md.pedTexture.getOverlayAssetFromId(IsPedMale(ped),layerName, data)
    data.id = nil
  end
  ...
  Md.timeout.delay('updatePedTexture',200, function()
    ...
    Md.utils.waiter(function() return not IsTextureValid(textureId) end)
    ...
    if IsTextureValid(textureId) then
      ...
      Entity(ped).state:set('Md_pedTexture',pedsTextures[ped])
      CreateThread(function()
        local textureId = textureId
        Md.utils.waiter(function() return IsPedReadyToRender(ped) end)
...
---@param category string
---@param layerName string
function Md.pedTexture.remove(ped,layerName)
  Md.pedTexture.apply(ped,layerName,{})
end

---@param ped integer
function Md.pedTexture.refreshAll(ped)
  pedsTextures[ped] = pedsTextures[ped] or Entity(ped).state['Md_pedTexture'] or {}
  ...
  for _,data in pairs (pedsTextures[ped]) do
    for layername,layer in pairs (data.layers) do
      Md.pedTexture.apply(ped,layername,layer)
    end
  end
end

function Md.pedTexture.overwriteCategory(ped,category,overlays)
  pedsTextures[ped] = pedsTextures[ped] or Entity(ped).state['Md_pedTexture'] or {}
  if pedsTextures[ped][category] then
    pedsTextures[ped][category].layers = {}
    for layername,cat in pairs (Md.pedTexture.categories) do
      if cat == category then
        Md.pedTexture.remove(ped,layername)
        break
      end
    end
  end
...
function Md.pedTexture.getAll(ped)
  local layers = {}
  pedsTextures[ped] = pedsTextures[ped] or Entity(ped).state['Md_pedTexture'] or {}
  for _,data in pairs (pedsTextures[ped]) do
    for layername,layer in pairs (data.layers) do
...
-- Entity(PlayerPedId()).state:set('Md_pedTexture',nil)

return Md.pedTexture
