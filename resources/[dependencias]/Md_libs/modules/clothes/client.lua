...
for _, category in pairs(Md.clothes.order) do
  Md.clothes.categoryName[joaat(category)] = category
end

Md.clothes.wearableStates = {
  shirts_full = {
...
local function getCategoryName(category)
  ...
  return Md.clothes.categoryName[category] or ('unknown:' .. category)
end
...
local function ResetCachedColor(ped, category)
  if not Md.cache.clothes.color[ped] then return end
  Md.cache.clothes.color[ped][category] = nil
  if category == `neckwear` then
    Md.cache.clothes.color[ped][`neckerchiefs`] = nil
  end
end
...
local function AddCachedClothes(ped, category, hash, palette, tint0, tint1, tint2)
  ...
  if not Md.cache.clothes.color[ped] then Md.cache.clothes.color[ped] = {} end
  Md.cache.clothes.color[ped][category] = {
  ...
  if category == `neckwear` then
    Md.cache.clothes.color[ped][`neckerchiefs`] = Md.cache.clothes.color[ped][category]
    Md.cache.clothes.color[ped][`neckerchiefs`].category = "neckerchiefs"
  elseif category == `neckerchiefs` then
    Md.cache.clothes.color[ped][`neckwear`] = Md.cache.clothes.color[ped][category]
    Md.cache.clothes.color[ped][`neckwear`].category = 'neckwear'
  end
end

---@param ped integer the entity ID
function PutInCacheCurrentClothes(ped)
  if Md.cache.clothes.color[ped] then return Md.cache.clothes.color[ped] end
  ...
  return Md.cache.clothes.color[ped]
end
...
local function ReapplyClothesStats(ped)
  local hash = 0
  for category, _ in pairs(Md.clothes.wearableStates) do
    local isEquiped, index = Md.clothes.isCategoryEquiped(ped, category)
...
local function ReapplyClothesColor(ped)
  for category, data in pairs(Md.cache.clothes.color[ped] or {}) do
    SetTextureOutfitTints(ped, category, data.palette, data.tint0, data.tint1, data.tint2)
  end
  Md.cache.clothes.color[ped] = nil
end

local function ReapplyCached(ped)
  if not Md.cache.clothes.color[ped] then return end
  Md.timeout.delay('ReapplyCachedColor', function() WaitRefreshPed(ped) end, function()
...
---@return boolean isMp
function Md.clothes.getComponentCategory(ped, hash)
  local isMp = true
...
function Md.clothes.apply(ped, category, data)
  ...
  if data.hash then
    categoryHash, isMp = Md.clothes.getComponentCategory(ped, data.hash)
  end
...
---@param ped integer the entity
---@param category string the clothes category
function Md.clothes.remove(ped, category)
  return Md.clothes.apply(ped, category, 0)
end
...
---@param state any
function Md.clothes.setWearableState(ped, category, hash, state)
  Entity(ped).state:set('wearableState:' .. category, state)
  PutInCacheCurrentClothes(ped)
  local data = formatClothesData(hash)
  if not data.hash then
    data.hash = Md.clothes.getComponentEquiped(Md.me,category)
  end
  UpdateShopItemWearableState(ped, data.hash, state)
  if category == "neckwear" and GetHashFromString(state) == `base` then
    Md.clothes.apply(ped, "beards_complete", Md.cache.clothes.color[ped][`beards_complete`])
  end
  RefreshPed(ped)
  ReapplyCached(ped)
end
...
---@param ped integer the entity
---@return boolean
function Md.clothes.neckwearIsUp(ped)
  return Entity(ped).state['wearableState:neckwear'] == Md.clothes.wearableStates.neckwear[1]
end

Md.clothes.isNeckweaUp = Md.clothes.neckwearIsUp

---@param ped integer the entity
---@return boolean
function Md.clothes.sleeveIsRolled(ped)
  return (Entity(ped).state['wearableState:shirts_full'] or ""):find('rolled') ~= nil
end

Md.clothes.isSleeveRolled = Md.clothes.sleeveIsRolled

---@param ped integer the entity
---@return boolean
function Md.clothes.collarIsOpened(ped)
  return (Entity(ped).state['wearableState:shirts_full'] or ""):find('open') and true or false
end

Md.clothes.isCollarOpened = Md.clothes.collarIsOpened

---@param ped integer the entity
---@param hash any the hash of the clothes
function Md.clothes.sleeveUnroll(ped, hash)
  if Md.clothes.isCollarOpened(ped) then
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[10])
  else
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[00])
  end
end

Md.clothes.unrollSleeve = Md.clothes.sleeveUnroll

---@param ped integer the entity
---@param hash any the hash of the clothes
function Md.clothes.sleeveRoll(ped, hash)
  if Md.clothes.isCollarOpened(ped) then
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[11])
  else
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[01])
  end
end

Md.clothes.rollSleeve = Md.clothes.sleeveRoll

---@param ped integer the entity
---@param hash any the hash of the clothes
function Md.clothes.collarOpen(ped, hash)
  if Md.clothes.isSleeveRolled(ped) then
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[11])
  else
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[10])
  end
...
Md.clothes.openCollar = Md.clothes.collarOpen

---@param ped integer the entity
---@param hash any the hash of the clothes
function Md.clothes.collarClose(ped, hash)
  if Md.clothes.isSleeveRolled(ped) then
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[01])
  else
    Md.clothes.setWearableState(ped, 'shirts_full', hash, Md.clothes.wearableStates.shirts_full[00])
...
Md.clothes.closeCollar = Md.clothes.collarClose

---@param ped integer the entity
---@return boolean
function Md.clothes.bootsAreUnderPant(ped)
  return Entity(ped).state['wearableState:boots'] == Md.clothes.wearableStates.boots[1]
end

Md.clothes.isBootsUnderPant = Md.clothes.bootsAreUnderPant

---@param ped integer the entity
---@return boolean
function Md.clothes.vestIsUnderPant(ped)
  return Entity(ped).state['wearableState:vests'] == Md.clothes.wearableStates.vests[1]
end

Md.clothes.isVestUnderPant = Md.clothes.vestIsUnderPant

---@param ped integer the entity
---@return boolean
function Md.clothes.loadoutIsOnRight(ped)
  return Entity(ped).state['wearableState:loadouts'] == Md.clothes.wearableStates.loadouts[1]
end

Md.clothes.isLoadoutOnRight = Md.clothes.loadoutIsOnRight

function Md.clothes.hairIsPomade(ped)
  return Entity(ped).state['wearableState:hair'] == Md.clothes.wearableStates.hair[1]
end
...
---@param ped integer the entity
---@return table
function Md.clothes.getCategoriesEquiped(ped)
  if Md.cache.clothes.getEquiped[ped] then
    return Md.cache.clothes.getEquiped[ped]
  end
  Md.cache.clothes.getEquiped[ped] = {}
  local numComponent = GetNumComponentsInPed(ped) or 0
  for index = 0, numComponent - 1 do
    --Get current clothes
    local category = GetCategoryOfComponentAtIndex(ped, index)
    Md.cache.clothes.getEquiped[ped][category] = {
      index = index,
      category = getCategoryName(category),
    }
    if category == `neckwear` then
      Md.cache.clothes.getEquiped[ped][`neckerchiefs`] = Md.cache.clothes.getEquiped[ped][category]
      Md.cache.clothes.getEquiped[ped][`neckerchiefs`].category = 'neckerchiefs'
    elseif category == `neckerchiefs` then
      Md.cache.clothes.getEquiped[ped][`neckwear`] = Md.cache.clothes.getEquiped[ped][category]
      Md.cache.clothes.getEquiped[ped][`neckwear`].category = 'neckwear'
    end
  end
  --clear cached value
  SetTimeout(500, function()
    Md.cache.clothes.getEquiped[ped] = nil
  end)
  return Md.cache.clothes.getEquiped[ped]
end
...
function Md.clothes.isCategoryEquiped(ped, category)
  local categoryHash = GetHashFromString(category)
  if not IsMetaPedUsingComponent(ped,categoryHash) then
    return false,0
  end
  local equiped = Md.clothes.getCategoriesEquiped(ped)
  if not equiped[categoryHash] then return false,0 end
  return true, equiped[categoryHash].index
end

function Md.clothes.getComponentEquiped(ped,category)
  local categoryHash = GetHashFromString(category)
  if not IsMetaPedUsingComponent(ped,categoryHash) then
    return false
  end
