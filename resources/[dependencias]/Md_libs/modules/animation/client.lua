...
Md.animation.easeIn = 4.0
Md.animation.easeOut = -4.

function Md.animation.load(dict,waiter)
...
function Md.animation.play(ped,dict,name,duration,flag,offset)
  if not duration then duration = -1 end
  if not flag then flag = 0 end
  if not offset then offset = 0.0 end
  Md.animation.load(dict,true)
  TaskPlayAnim(ped, dict, name, Md.animation.easeIn, Md.animation.easeOut, duration, flag, offset, false, false, false)
...
function Md.animation.goToCoords(ped,coords,speed,waiter,distanceToStop)
  speed = speed or 1.0
  if waiter or type(coords) == "vector4" then
    ...
    if type(coords) == "vector4" then
      Md.animation.setDesiredHeading(ped,coords.w,true)
    end
  else
...
function Md.animation.setDesiredHeading(ped,heading,waiter)
