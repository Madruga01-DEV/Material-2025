...
AddEventHandler('onResourceStop', function(resourceName)
	...
	for entity,_ in pairs (Entities) do
		Md.entity.delete(entity)
	end
end)

---@param entity integer
function Md.entity.requestControl(entity)
...
function Md.entity.fadeAndDelete(entity,duration)
	duration = duration or 1000
	if not DoesEntityExist(entity) then return end

	Md.entity.fadeOut(entity,duration)
	Md.entity.delete(entity)
end

---@param entity integer
---@param duration? integer duration in ms. default (1000)
function Md.entity.fadeIn(entity,duration)
	duration = duration or 1000
	if not DoesEntityExist(entity) then return end
...
---@param entity integer
---@param duration? integer duration in ms. default (1000)
function Md.entity.fadeOut(entity,duration)
	duration = duration or 1000
	local startTime = GetGameTimer()
	local startAlpha = GetEntityAlpha(entity)
	local alpha = startAlpha
...
function Md.entity.create(model,coords,heading,networked,fadeDuration)
	networked = networked or false
	fadeDuration = fadeDuration or 0
	local model = GetHashFromString(model)
	Md.utils.loadGameData(model,true)
	local entity
	...
	if fadeDuration > 0 then
		Md.entity.fadeIn(entity,fadeDuration)
	end
